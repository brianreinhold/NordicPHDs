...
bool createSystemInfoData(s_SystemInfoData** systemInfoDataPtr, s_SystemInfo *systemInfo)
{
    ...
    if (systemInfoData == NULL)
    {
        NRF_LOG_DEBUG("Could not allocate memory for the System Info Index struct.");
        return false;
    }
    ...
    if (systemInfoData->systemInfoBuf == NULL)
    {
        NRF_LOG_DEBUG("Could not allocate memory for the System Info data buffer.");
        free(systemInfoData);
        return false;
    }
    // Command
...
bool createMsmtGroup(s_MsmtGroup **msmtGroupPtr, bool hasTimeStamp, unsigned char numberOfMetMsmts, unsigned char groupId)
{
    ...
    if (msmtGroup->metMsmt == NULL)
    {
        NRF_LOG_DEBUG("Could not allocate memory for the %d Msmt group s_MetMsmt pointers.", numberOfMetMsmts);
        free (msmtGroup);
        return false;
    }
...
short addMetMsmtToGroup(s_MetMsmt *metMsmt, s_MsmtGroup **msmtGroupPtr)
{
    ...
    {
        NRF_LOG_DEBUG("Number of MetMsmts %d exceeds numberOfMetMsmts %d.", (msmtGroup->header->currentMsmtCount + 1), msmtGroup->header->numberOfMsmts);
        return -1;
    }
    ...
    unsigned short numberOfSamples, unsigned char sampleSize)
{
    ...
    if (metMsmt == NULL)
    {
        NRF_LOG_DEBUG("Unable to allocate memory for RTSA Met Measurement.");
        return false;
    }
    metMsmt->type = type;
