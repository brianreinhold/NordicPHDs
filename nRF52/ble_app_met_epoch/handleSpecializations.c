...
void configureSpecializations(void)
{
    ...
    cleanUpSystemInfo(&systemInfo);     // Now we can cleanup and free the resources taken by the s_SystemInfo. We don't need it anymore.
        ...
        // session
        result = createMsmtGroup(&spiroSessionGroup, (USES_TIMESTAMP == 1), 1, group_id++); // 1 msmt
        result = createCodedMsmt(&session, MDC_DIAG_SESSION_SPIRO);
        session_index = addMetMsmtToGroup(session, &spiroSessionGroup);
        createMsmtGroupDataArray(&msmtGroupSpiroSessionData, spiroSessionGroup, sMetTime, PACKET_TYPE_NORMAL);
        session_id = msmt_id;
        ...
        // streaming data
        result = createMsmtGroup(&spiroStreamingGroup, (USES_TIMESTAMP == 1), 1, group_id++); // 1 msmt
      ...
      //  result = createRtsaMsmt(&volume, MDC_VOL_AWAY, MDC_DIM_MILLI_L, &period, &scaleFactor, &offset, NO_OF_SAMPLES, SAMPLE_SIZE);
        result = createRtsaMsmt(&flow, MDC_FLOW_AWAY, MDC_DIM_MILLI_L_PER_SEC, &period, &scaleFactor, &offset, NO_OF_SAMPLES, SAMPLE_SIZE);
      //  volume_index = addMetMsmtToGroup(volume, &spiroStreamingGroup);
        ...
        // maneuver results
        result = createMsmtGroup(&spiroManeuvGroup, (USES_TIMESTAMP == 1), 8, group_id++); // 8 msmts for now
        setHeaderRefs(&spiroManeuvGroup, 1);
        result = createNumericMsmt(&fev1, MDC_VOL_AWAY_EXP_FORCED_1S, false, MDC_DIM_L);
        result = createNumericMsmt(&fev6, MDC_VOL_AWAY_EXP_FORCED_6S, false, MDC_DIM_L);
        result = createNumericMsmt(&fvc, MDC_VOL_AWAY_EXP_FORCED_CAPACITY, false, MDC_DIM_L);
        result = createNumericMsmt(&pef, MDC_FLOW_AWAY_EXP_FORCED_PEAK, false, MDC_DIM_L_PER_SEC);
        result = createNumericMsmt(&fet, MDC_VOL_AWAY_EXP_FORCED_TIME, false, MDC_DIM_SEC);
        result = createNumericMsmt(&fev1z, MDC_VOL_AWAY_FEV1_Z_SCORE, false, MDC_DIM_DIMLESS);
        result = createNumericMsmt(&fev1_lln, MDC_VOL_AWAY_FEV1_LLN, false, MDC_DIM_L);
        result = createNumericMsmt(&fev1_percent_pred,  MDC_VOL_AWAY_FEV1_PERCENT_PRED, false, MDC_DIM_PERCENT);
        ...
        createMsmtGroupDataArray(&msmtGroupSpiroManeuvData, spiroManeuvGroup, sMetTime, PACKET_TYPE_NORMAL);
        ...
        result = createMsmtGroup(&spiroSummaryGroup, (USES_TIMESTAMP == 1), 2, group_id++); // 1 msmts
        setHeaderRefs(&spiroSummaryGroup, 1);
        result = createCodedMsmt(&fvcAtsGrade, MDC_SPIRO_FVC_ATS_QUAL);
        setMetMsmtRefs(&fvcAtsGrade, 2);
        result = createCodedMsmt(&fev1AtsGrade, MDC_SPIRO_FEV1_ATS_QUAL);
        ...
        // session end
        result = createMsmtGroup(&spiroSessionEndGroup, (USES_TIMESTAMP == 1), 1, group_id++); // 1 msmt
        setHeaderRefs(&spiroSessionEndGroup, 1);
        result = createCodedMsmt(&sessionEnd, MDC_DIAG_SESSION_SPIRO);
...
bool encodeSpecializationMsmts(s_MsmtData *msmt)
{
    ...
    // except for the time stamps.
        ...
        if (spiro_sequence == 0)
        {
            if (!prepareMeasurements(msmtGroupSpiroSessionData)) return false;
            // Add the command
        ...
        if (spiro_sequence == 1)
        {
            if (!prepareMeasurements(msmtGroupSpiroSettingsData)) return false;
            // Add the command
        ...
        {
            if (!prepareMeasurements(msmtGroupSpiroSubSessionData)) return false;
            // Add the command
        ...
        else if (spiro_sequence == 3)
        {
            ...
            if (!prepareMeasurements(msmtGroupSpiroStreamData)) return false;
            // Add the command
        ...
        {
            if (!prepareMeasurements(msmtGroupSpiroManeuvData)) return false;
            // Add the command
        ...
        {
            if (!prepareMeasurements(msmtGroupSpiroSummaryData)) return false;
            // Add the command
