...
The bulk of the code work is encoding and decoding the packets which requires no platform-specific APIs and can be done with standard C on any platform. The platform independence of the packet work means this code would be quite simple to port to different BLE MCUs. In fact, this MPM standard could be supported by any reliable transport, for example, USB.
